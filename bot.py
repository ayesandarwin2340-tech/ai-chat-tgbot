...
"""
JANAB – Smart Assistant with Gemini + Subscription Model
...
SESSION_NAME = "janab_gemini_session"

# ⚠️ PUT YOUR REAL GEMINI API KEY HERE
GEMINI_API_KEY = "AIzaSyCVIMpCVNet9OOwHGzIq01H6s8kIKdeJ34"
...
if not GEMINI_API_KEY or GEMINI_API_KEY.startswith("PUT_"):
    raise RuntimeError("Please set the GEMINI_API_KEY in the code.")
...
# -----------------------------
# PERSONA & SYSTEM PROMPT (Translated to English)
# -----------------------------
BASE_PERSONA = """
...
async def handler(event: events.NewMessage.Event):
    ...
    is_owner = (sender is not None and int(sender.id) == int(OWNER_CHAT_ID))
    ...
    # --- GROUP CHAT (Only reply if mentioned or keyword used) ---
    if event.is_group or event.is_channel:
        if "Janab" in text or "janab" in text or (event.is_reply and event.reply_to_msg_id):
            # Also reply if replying to bot's message (simplified logic)
...
async def main():
    load_memory()
    print("Janab AI is starting...")
    await client.start()
    print("✅ Janab is Online! Waiting for messages...")
